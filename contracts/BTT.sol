...
/**
 * @title BTT(Batch Token Transfer), support ETH and ERC20 Tokens
 * @dev To Use this Dapp: https://btt.kols.club
*/
...
contract BTT is Ownable{
    ...
    uint public txFee = 0.0003 ether;
  ...
  function removeFromVIPList(address[] _vipList) onlyOwner public {
   ...
   function ethSendSameValue(address[] _to, uint _value) internal {
        ...
        if(vip){
            ...
            require(remainingValue >= sendAmount.add(txFee.mul(_to.length-1))) ;
        }
	    ...
	    emit LogTokenMultiSent(0x000000000000000000000000000000000000aaaa,msg.value);
    }
    ...
    function coinSendSameValue(address _tokenAddress, address[] _to, uint _value)  internal {
      ...
      bool vip = isVIP(msg.sender);
          if(!vip){
  		    require(sendValue >= txFee.mul(_to.length-1));
          }
  		require(_to.length <= 255);
    ...
    function batchEth(address[] _to, uint _value) payable public {
		ethSendSameValue(_to,_value);
	  }

    function batchSend(address[] _to, uint[] _value) payable public {
		 ethSendDifferentValue(_to,_value);
	  }



    function batchToken(address _tokenAddress, address[] _to, uint[] _value) payable public {
	    coinSendDifferentValue(_tokenAddress, _to, _value);
    }

    function airDrop(address _tokenAddress, address[] _to, uint _value)  payable public {
