...
contract KOLLockNode is Ownable{
    ...
    mapping (address => uint256) nodeBalance;
    ...
    modifier onlyNodes {
      require(token.queryNode(msg.sender) || token.querySuperNode(msg.sender));
      _;
    }
    ...
    function queryRatePPT() private returns(uint256){
      return token.userSupplyed.mul(1000).div(totalUserSupply);
    }

    function putBalace(address _coreAddr,uint256 _balance) public {
    // transfer tokens to address
    }
    function getKOL(uint256 _amount) public {
      // get KOL Tokens from the contract
    }
